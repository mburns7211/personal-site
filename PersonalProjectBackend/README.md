...
```bash
PersonalProjectBackend$ sam build --use-container
```
...
```bash
PersonalProjectBackend$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
PersonalProjectBackend$ sam local start-api
PersonalProjectBackend$ curl http://localhost:3000/
```
...
```bash
PersonalProjectBackend$ sam logs -n HelloWorldFunction --stack-name PersonalProjectBackend --tail
```
...
```bash
PersonalProjectBackend$ pip install -r tests/requirements.txt --user
# unit test
PersonalProjectBackend$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
PersonalProjectBackend$ AWS_SAM_STACK_NAME="personalprojectbackend" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "personalprojectbackend"
```
